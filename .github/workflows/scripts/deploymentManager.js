module.exports = class DeploymentManager {
              ...
              if (currentState !== 'inactive') {
                ...
                if (currentState !== 'failure') {
                  return this.transitionPrDeploymentToFailure(deployment)
                    ...
                    .then(this.sleep(15))
                    .then(() => {
                      core.info(`Deployment: ${deployment.id}:${deployment.environment} transitioning to inactive, ${Date.now()}`);
  ...
  async transitionPrDeploymentToFailure(deployment) {
    ...
    core.info(`Deployment: ${deployment.id}:${deployment.environment} transitioning to failure, ${Date.now()}`);
